...
const EventInfoForm = forwardRef<EventInfoFormRef, EventInfoFormProps>(({ onEventCreated }, ref) => {
  ...
  const tagOptions = eventTagsData?.data?.eventTags?.map((tag) => ({
  ...
  const handleNextStep = async (): Promise<boolean> => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : '活動建立過程中發生錯誤');
      return false;
    } finally {
      // 無論成功或失敗都要重設載入狀態
  ...
  return (
    <div className="flex flex-col gap-8 self-stretch">
      <h1 className="text-3xl font-semibold">創建活動</h1>
        ...
        <div className="flex flex-col self-stretch gap-2">
          ...
          >
            <textarea
              ...
              className="w-full border border-gray-400  rounded-2xl p-4 resize-none font-sans text-base"
          ...
          >
            <div className="flex flex-wrap gap-6">
              {eventTagsLoading ? (
                ...
                <div className="text-red-500">載入標籤失敗，請重新整理頁面</div>
              ) : (
                <FormCheckboxGroup
                  name="tags"
                  options={tagOptions}
